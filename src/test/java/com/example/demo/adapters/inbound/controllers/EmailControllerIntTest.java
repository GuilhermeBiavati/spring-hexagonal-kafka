...
class EmailControllerIntTest {
    ...
    void canSave() throws Exception {
        ...
        // given
        String ownerRef = String.format("%s %s", faker.name().firstName(), faker.name().lastName());
        String emailTo = String.format("%s@gmail.com", StringUtils.trimAllWhitespace(faker.name().firstName()));
        String emailFrom = String.format("%s@gmail.com", StringUtils.trimAllWhitespace(faker.name().firstName()));
        String subject = String.format("%s %s", faker.name().firstName());
        String text = String.format("%s %s", faker.name().firstName(), faker.name().lastName());

        Email email = new Email(ownerRef, emailTo, emailFrom, subject, text);

        // when
        ResultActions resultActions = mockMvc.perform(post("/sending-email").contentType(MediaType.APPLICATION_JSON)
