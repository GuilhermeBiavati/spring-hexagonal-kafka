...
public class EmailServiceEmplTest {
    ...
    @BeforeEach
    void setUp() {
        underTest = new EmailServiceImpl(emailRepositoryPort, sendEmailServicePort, sendKafkaServicePort);
    }
    ...
    void cantSendEmail() {
        // given
        Email email = new Email("Guilherm", "guilhermebiavati@gmail.com", "guilhermebiavati@gmail.com", "sub", "text");
        doThrow(new ArrayIndexOutOfBoundsException()).when(sendEmailServicePort).sendEmailSmtp(email);
        // when
        underTest.sendEmail(email);
        // then
        Assertions.assertThatThrownBy(() -> sendEmailServicePort.sendEmailSmtp(email)).isInstanceOf(Exception.class);
        ArgumentCaptor<Email> emailArgumentCaptor = ArgumentCaptor.forClass(Email.class);
        verify(emailRepositoryPort).save(emailArgumentCaptor.capture());
        Email captEmail = emailArgumentCaptor.getValue();
    ...
    void willThrowWhenSendKafka() {
        ...
        Assertions.assertThatThrownBy(() -> underTest.sendKafka(email)).isInstanceOf(Exception.class);
